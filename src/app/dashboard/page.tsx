...
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { PlusCircle, Settings, ArrowRight, Loader2, Compass, MessageSquare, BookOpen, Star } from "lucide-react";
...
export default function DashboardPortal() {
  ...
  // 👑 ADMIN VIEW (Your original Host Console cards)
  if (role === "admin") {
    return (
      <main className="min-h-screen pt-24 pb-12 bg-slate-50/50">
        <div className="max-w-4xl mx-auto px-4">
  ...
  // 💼 USER VIEW (New Custom member experience)
  return (
    <main className="min-h-screen pt-24 pb-12 bg-slate-50/50">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Member Portal</span>
          </div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Welcome back, Member</h1>
          <p className="text-sm text-slate-500 mt-1">Book professional hybrid desks, manage your custom bookings, or reach support.</p>
        </div>

        {/* Action Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          
          {/* Card 1: Explore Workspaces */}
          <div 
          ...
          >
            <div>
              ...
              <h2 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition">Explore Workspaces</h2>
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">Browse active locations, view premium community spaces, and secure a hot desk or boardroom.</p>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-bold text-indigo-600 mt-4">
              Find a Workspace <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition" />
            </div>
          </div>

          {/* Card 2: Provide Feedback */}
          <div 
            onClick={() => router.push("/feedback")}
            className="group bg-white border border-slate-200/70 p-8 rounded-2xl shadow-sm hover:shadow-md hover:border-indigo-500/50 transition duration-300 cursor-pointer flex flex-col justify-between h-56"
          >
            <div>
              <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center mb-5 group-hover:bg-emerald-600 group-hover:text-white transition duration-300">
                <MessageSquare className="w-6 h-6" />
              </div>
              <h2 className="text-lg font-bold text-slate-900 group-hover:text-emerald-600 transition">Submit Support Feedback</h2>
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">Submit bugs, request dynamic features, or send general feedback to help shape our network.</p>
            </div>
            <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-600 mt-4">
              Open Feedback Form <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-1 transition" />
            </div>
          </div>
